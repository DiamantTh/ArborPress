...
async def _get_footer_pages() -> list:
    ...
    try:
        from arborpress.models.content import Page, PageType
...
async def index():
    ...
    try:
        async for db in get_db_session():
            ...
            total_stmt = select(Post).where(Post.status == PostStatus.published)
...
async def tag_archive(tag: str):
    ...
    async for db in get_db_session():
        ...
        posts_stmt = (
            select(Post)
            .where(Post.status == PostStatus.published)
            .where(Post.tags.contains(tag_obj))
            .order_by(Post.published_at.desc())
        )
...
async def media_serve(yyyy: int, mm: int, filename: str):
    ...
    cfg = get_settings()
    media_root = Path(cfg.web.base_url.split("//")[-1].split("/")[0])  # Fallback
    # Besser: media_root aus config lesen (optional: cfg.web erweiterbar)
    media_path = Path("media") / str(yyyy) / f"{mm:02d}" / safe_name
...
async def post_comment_submit(slug: str):
    ...
    async for db in get_db_session():
        ...
        # Bestätigungs-Mail senden (wenn Backend != none)
        if comments_section.get("require_email_confirmation", True):
